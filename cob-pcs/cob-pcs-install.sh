...
# Script for installing apt packages on all client_pcs.

# checking input parameters
if [ "$#" -lt 1 ]; then
	echo -e "${red}ERROR: Wrong number of parameters${NC}"
	echo -e "${red}Usage: $SCRIPT <packages_to_install>${NC}"
	exit 1
fi
...
for client in $pc_list; do
	echo -e "${green}-------------------------------------------${NC}"
	echo -e "${green}Updating $client ${NC}"
	echo -e "${green}-------------------------------------------${NC}"
	echo ""
	...
	ret=${PIPESTATUS[0]}
	if [ "$ret" != 0 ] ; then
		echo -e "${red}apt-get return an error (error code: $ret), aborting...${NC}"
		exit 1
	fi
	echo ""
	echo -e "${green}-------------------------------------------${NC}"
	echo -e "${green}Installing <<""$*"">> on $client ${NC}"
	echo -e "${green}-------------------------------------------${NC}"
	echo ""
	# shellcheck disable=SC2029
