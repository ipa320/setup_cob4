...
In order to allow the non-root users execute the cob-start and cob-stop commands, it is posible create a +upstart+ group which  will be set to execute the commands. 

Create the group

----

sudo addgroup upstart

----

Modify the sudo rights adding the following lines to +/etc/sudoers+(to edit this file use the command +sudo visudo+) on all robot pcs:

----

%upstart ALL=NOPASSWD:/usr/sbin/cob-start
%upstart ALL=NOPASSWD:/usr/sbin/cob-stop
